...
The current status is experimental. `aic build compiler/aic/tools/aic_selfhost` produces a real stage0 compiler, stage0 links the existing runtime C source parts for native executable materialization, and stage0 can check smaller compiler packages such as `compiler/aic/libs/source`. The backend-covered stage0 surface includes primitive functions, backend-covered aggregate signatures, return-position struct literals, lossless fixed-width integer widening at return boundaries, runtime-backed string replacement, runtime-backed filesystem result construction for direct `read_text`/`temp_file`/`write_text`/`delete` returns, runtime-backed argument lookup for the canonical optional argument-defaulting match, return-position primitive/string conditionals, and stdout/stderr string printing. The latest bootstrap report keeps stage1 failing with backend `E5101` coverage gaps in executable statement lowering, match/loop lowering, field/local value lowering, and imported compiler package emission. The full stage1/stage2 loop remains unsupported until stage0 can compile the complete compiler package graph, emit a runnable stage1 compiler, stage1 can emit stage2, and the stage1/stage2 artifact digests match or have documented stable differences covered by tests.
