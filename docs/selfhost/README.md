...
Class decisions are machine-verifiable through `rust_path_classes[*].retirement_decision`. Rust reference compiler classes must use `remove-after-replacement`, while retained Rust host/tooling/test classes must name a non-reference role and provide evidence for each required command before approval. For report-producing commands, the audit verifies the expected self-host parity, bootstrap, and stage-matrix report formats instead of accepting arbitrary checksum-bearing files. After removal, a `retired` manifest expects approved removal class paths to be absent from the repository.

Generate review entries with `scripts/selfhost/retirement_evidence.py` after the corresponding commands have run. The helper records checksums for bake-in, rollback, and class decision evidence and can assemble a candidate manifest under `target/selfhost-retirement/`. After a successful local `make release-preflight`, `make selfhost-retirement-bake-in-evidence` generates the current host bake-in entry, candidate manifest, and candidate audit report from the real local outputs. To validate the rollback source, set `AIC_SELFHOST_ROLLBACK_REF` to the approved tag or branch and run `make selfhost-retirement-rollback-evidence`; the target restores the rollback paths in a temporary worktree before recording build, audit, and marker-scan checksums.

Use `--path-base <bundle>` when creating evidence entries and `python3 scripts/selfhost/retirement_audit.py --evidence-root <bundle>` when auditing a candidate manifest whose reports, logs, and compiler artifacts are stored in a separate release evidence bundle. When Linux and macOS bake-in runs were captured under different roots, normalize them into one bundle with `normalize-bake-in-entry` and `normalize-rollback-entry` before calling `assemble-manifest --approver <release-approver>`.
